...
class FilterController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if ($properties->count()) {
            foreach ($properties as $propertyCategory) {
        ...
        return response()->json($this->setResponse('fail', [], 'Não foi possível retornar nenhum dado.'));
    ...
    public function category(Request $request)
    {
        ...
        return response()->json($this->setResponse('fail', [], 'Não foi retornado nenhum dado para essa pesquisa!'));
    ...
    public function garage(Request $request)
    {
        ...
        if ($priceBaseProperties->count()) {
            foreach ($priceBaseProperties as $property) {
                $priceBase[] = 'À partir de R$ ' . number_format($property->price, 2, ',', '.');
            }

            $collect = collect($priceBase);
            return response()->json($this->setResponse('success', $collect->unique()->toArray()));
        }
